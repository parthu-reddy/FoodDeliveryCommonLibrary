...
/**
 * Lets the test verifier actually deliver a contract's outputMessage to the
 * embedded broker.
 *
...
public class KafkaStubMessageSender implements MessageVerifierSender<Message<?>> {
    ...
    public <T> void send(T payload, Map<String, Object> headers, String destination, YamlContract contract) {
        try {
            ...
            throw new IllegalStateException("Failed to serialize or publish stub message to " + destination, e);
        }
    }
}
